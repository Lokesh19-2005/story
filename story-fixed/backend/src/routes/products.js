...
function enrichProduct(p, db) {
  const sizes  = db.prepare('SELECT size FROM product_sizes WHERE product_id=? ORDER BY rowid').all(p.id).map(r => r.size);
  ...
  return { ...p, sizes, colors, brand: brand?.name || '', category_slug: cat?.slug, category_label: cat?.label };
}

router.get('/', (req, res) => {
  const db = getDB();
  const { category, brand, search, sort = 'newest', page = 1, limit = 20 } = req.query;
  let q = 'SELECT p.* FROM products p LEFT JOIN brands b ON p.brand_id=b.id LEFT JOIN categories c ON p.category_id=c.id WHERE p.is_active=1';
  const args = [];
...
router.get('/:slug/related', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM products WHERE category_id=? AND id!=? AND is_active=1 LIMIT 4').all(p.category_id, p.id);
